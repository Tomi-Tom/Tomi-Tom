...
int wait_turn(tabcase **tab, tabcase **e_tab, int mode)
{
    ...
    stock_msg = postbox();
    if (is_valid_pick(stock_msg) == 84)
        return 84;
    tab[(int) (stock_msg[1] - '1')][(int) (stock_msg[0] - 'A')].is_touched = 1;
    my_putstr(stock_msg);
    my_putstr(": ");
    if (tab[(int) (stock_msg[1] - '1')][(int) (stock_msg[0] - 'A')].isboat > 0) {
        message("hit", status->enemy_pid);
        my_putstr("hit\n");
        tab[(int) (stock_msg[1] - '1')][(int) (stock_msg[0] - 'A')].display = 'x';
    } else {
        message("0", status->enemy_pid);
        tab[(int) (stock_msg[1] - '1')][(int) (stock_msg[0] - 'A')].display = 'o';
        my_putstr("missed\n");
    }
    return 1;
}
